namespace WSOptimizer7.Controllers
{
    ...
    public class CalculoOptimizadoV2Controller : Controller
    {
        private Dictionary<string, TablaModel> dataRegistroCache = new Dictionary<string, TablaModel>();
        ...
        private RespOptimizerModel GetOptimizerModelNOriginal(RequestOptimizerModel objReqDataIni)
        {
        ...
        [HttpPost]
        [Route("api/optimizado/v2")]
        public RespOptimizerModel GetOptimizerModelNV2([FromBody] RequestOptimizerModel objReqDataIni)
        {
            dataRegistroCache.Clear();
            string strSQLRef = "SELECT * FROM CatOptimizerC_Referencias ";
            dtRef = Database.execQuery(strSQLRef);
            ...
            Dictionary<int, ProductoModel> productosByCve = GetProductosByKey(objReqDataIni.Productos, p => p.CveProducto);
            Dictionary<int, ProductoModel> productosByPosicion = GetProductosByKey(objReqDataIni.Productos, p => p.Posicion);
            Dictionary<int, double> remainingMinDurations = GetRemainingDurationsByProduct(objReqDataIni.Productos, p => p.DuracionMin);
            Dictionary<int, double> remainingMaxDurations = GetRemainingDurationsByProduct(objReqDataIni.Productos, p => p.DuracionMax);
            ...
            int cveParametroSeleccionado = objReqDataIni.CveParametro;
            List<TablaModel> lstProdIni = GetEtapasIniciales(objReqDataIni);
            if (lstProdIni.Max(r => r.Edad_Final) > objReqDataIni.EdadSalida)
            {
                throw new Exception("La Edad de salida deseada es menor a la edad de salida real, verifique datos de entrada y etapas anteriores a Iniciador");
            }
            ...
            TablaModel prod5 = GetTablaOptimizerxProd5(objReqDataIni, 5, lstProdIni);
            lstProdIni.Add(prod5);
            double pesoFinal5 = prod5.Peso_Final;
            productosByPosicion[5].PesoFinal = pesoFinal5;

            VarMinMaxModel valMinMax5 = new VarMinMaxModel(prod5.Peso_Final, 0, prod5.Edad_Final, 0);
            VarMinMaxModel valMinMax6 = GetMinMaxValuesV2(objReqDataIni, valMinMax5, 6, remainingMinDurations, productosByPosicion);
            VarMinMaxModel valMinMax7 = GetMinMaxValuesV2(objReqDataIni, valMinMax6, 7, remainingMinDurations, productosByPosicion);
            VarMinMaxModel valMinMax8 = GetMinMaxValuesV2(objReqDataIni, valMinMax7, 8, remainingMinDurations, productosByPosicion);

            ProductoModel prod6 = productosByCve[6];
            ProductoModel prod7 = productosByCve[7];
            ProductoModel prod8 = productosByCve[8];
            ProductoModel prod9 = productosByCve[9];
            ProductoModel prod10 = productosByCve[10];
            ...
            for (int k = diasTmpRactopamina; k <= diasTopeRactopamina; k += 1)
            {
                for (double x = valMinMax6.ValorMinimo; x <= valMinMax6.ValorMaximo; x += step6)
                {
                    objReqDataIni.IsOptimizar = true;
                    objReqDataIni.PesoInicialTmp = prod5.Peso_Final;
                    objReqDataIni.EdadInicialTmp = prod5.Edad_Final;
                    ...
                    TablaModel tModel6 = GetDataRegistroCached(prod6, objReqDataIni);
                    while (Math.Round(tModel6.Duracion_Etapa) < tModel6.Duracion_Minima)
                    {
                        x += 1;
                        prod6.PesoFinal = x;
                        tModel6 = GetDataRegistroCached(prod6, objReqDataIni);
                        if (Math.Round(tModel6.Duracion_Etapa) > tModel6.Duracion_Minima)
                        {
                            x -= 1;
                            prod6.PesoFinal = x;
                    ...
                    for (double y = valMinMax7.ValorMinimo; y <= valMinMax7.ValorMaximo; y += step7)
                    {
                        if (y < x)
                            y = x;
                        ...
                        TablaModel tModel7 = GetDataRegistroCached(prod7, objReqDataIni);
                        while (Math.Round(tModel7.Duracion_Etapa) + 1 < tModel7.Duracion_Minima)
                        {
                            y += 1;
                            prod7.PesoFinal = y;
                            tModel7 = GetDataRegistroCached(prod7, objReqDataIni);
                        }
                        ...
                        for (double z = y; z <= valMinMax8.ValorMaximo; z += step8)
                        {
                            objReqDataIni.PesoInicialTmp = tModel7.Peso_Final;
                            objReqDataIni.EdadInicialTmp = tModel7.Edad_Final;
                            TablaModel tModel8;
                            ...
                            {
                                prod8.PesoFinal = z;
                                tModel8 = GetDataRegistroCached(prod8, objReqDataIni);
                                if (Math.Round(tModel8.Duracion_Etapa) > tModel8.Duracion_Maxima)
                                    break;
                            ...
                            double edadTopeObjetivo = objReqDataIni.EdadVenta - k;
                            double duracionEtapa8 = Math.Round(tModel8.Duracion_Etapa);
                            if (duracionEtapa8 >= tModel8.Duracion_Minima && duracionEtapa8 <= tModel8.Duracion_Maxima)
                            {
                                TablaModel tModel9 = GetPeso_FinalEtapa9_10(objReqDataIni, tModel8, prod9, edadTopeObjetivo);
                                if (tModel9 != null)
                                {
                                    TablaModel tModel10 = GetPeso_FinalEtapa9_10(objReqDataIni, tModel9, prod10, objReqDataIni.EdadVenta);
                                    if (tModel10 != null && !(IsFueraRango(tModel6) || IsFueraRango(tModel7) || IsFueraRango(tModel8) || IsFueraRango(tModel9) || IsFueraRango(tModel10)))
                                    {
                                        productosByPosicion[6].PesoFinal = x;
                                        productosByPosicion[7].PesoFinal = y;
                                        productosByPosicion[8].PesoFinal = tModel8.Peso_Final;
                                        productosByPosicion[9].PesoFinal = tModel9.Peso_Final;
                                        productosByPosicion[10].PesoFinal = tModel10.Peso_Final;
                                        objReqDataIni.IsOptimizar = false;

                                        List<TablaModel> tablaOpt = new List<TablaModel>(lstProdIni.Count + 5);
                                        tablaOpt.AddRange(lstProdIni);
                                        tablaOpt.Add(tModel6);
                                        tablaOpt.Add(tModel7);
                                        tablaOpt.Add(tModel8);
                                        ...
                                        ResponseOptModel objR = new ResponseOptModel
                                        {
                                            Optimizer = GetOptimizer(objReqDataIni, tablaOpt)
                                        };
                                        objR.LstProducto.Add(new CatalogoModel("5", pesoFinal5.ToString()));
                                        objR.LstProducto.Add(new CatalogoModel("6", x.ToString()));
                                        objR.LstProducto.Add(new CatalogoModel("7", y.ToString()));
            ...
            RespOptimizerModel objRespModel = new RespOptimizerModel();
            objReqDataIni.IsOptimizar = false;
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 1, true), objReqDataIni, 1, productosByCve));
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 2, false), objReqDataIni, 2, productosByCve));
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 3, false), objReqDataIni, 3, productosByCve));
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 4, false), objReqDataIni, 4, productosByCve));
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 5, true), objReqDataIni, 5, productosByCve));
            objRespModel.ResponseParametro.Add(GetOptimizadoResponseV2(GetBestResponse(lstResult, 6, true), objReqDataIni, 6, productosByCve));
            ...
            Console.WriteLine("Datos fin v2");
            return objRespModel;
        }
        ...
        private ResponseOptimizerModel GetOptimizadoResponseV2(ResponseOptModel objRespModelOpt, RequestOptimizerModel objReqDataIni, int cveParametro, Dictionary<int, ProductoModel> productosByCve)
        {
            objRespModelOpt.LstProducto.ForEach(r =>
            {
                productosByCve[Int32.Parse(r.Clave)].PesoFinal = double.Parse(r.Valor);
            });

            objReqDataIni.CveParametro = cveParametro;
            ResponseOptimizerModel respOpt = GetDataOptizerService(objReqDataIni);
            respOpt.CveParametro = cveParametro;
            respOpt.Parametro = GetParametroEcoVal(cveParametro, "NomParametro");
            return respOpt;
        }

        private TablaModel GetDataRegistroCached(ProductoModel prod, RequestOptimizerModel reqModel)
        {
            if (!reqModel.IsOptimizar)
                return GetDataRegistro(prod, reqModel);

            string cacheKey = BuildDataRegistroCacheKey(prod, reqModel);
            if (dataRegistroCache.TryGetValue(cacheKey, out TablaModel cached))
                return CloneTablaModel(cached);

            TablaModel calculated = GetDataRegistro(prod, reqModel);
            dataRegistroCache[cacheKey] = CloneTablaModel(calculated);
            return calculated;
        }

        private static string BuildDataRegistroCacheKey(ProductoModel prod, RequestOptimizerModel reqModel)
        {
            return string.Join("|",
                prod.CveProducto,
                prod.PesoFinal.ToString("R"),
                reqModel.PesoInicialTmp.ToString("R"),
                reqModel.EdadInicialTmp.ToString("R"),
                reqModel.CveReferencia,
                reqModel.CveEstado,
                reqModel.Temperatura.ToString("R"),
                reqModel.MetrosCerdos.ToString("R"),
                reqModel.Desperdicio.ToString("R"),
                reqModel.EdadVenta.ToString("R"));
        }

        private static TablaModel CloneTablaModel(TablaModel source)
        {
            return new TablaModel
            {
                Identificador = source.Identificador,
                Costo = source.Costo,
                Ractopamina = source.Ractopamina,
                SIDLys_GDP = source.SIDLys_GDP,
                EM_Mixto = source.EM_Mixto,
                EM_KCal = source.EM_KCal,
                En_Kcal = source.En_Kcal,
                EN_EM = source.EN_EM,
                SIDLysPorc = source.SIDLysPorc,
                CDA_TA = source.CDA_TA,
                CDA_Espacio = source.CDA_Espacio,
                Max_Energia = source.Max_Energia,
                Min_Espacio = source.Min_Espacio,
                TC_min = source.TC_min,
                Ajuste_Temp = source.Ajuste_Temp,
                CDA_Kg = source.CDA_Kg,
                PresupuestoCerdo = source.PresupuestoCerdo,
                GDP = source.GDP,
                GDP_Curva = source.GDP_Curva,
                Peso_Inicial = source.Peso_Inicial,
                Peso_Final = source.Peso_Final,
                CA_GDP = source.CA_GDP,
                CA = source.CA,
                Edad_Inicial = source.Edad_Inicial,
                Edad_Final = source.Edad_Final,
                Duracion_Etapa = source.Duracion_Etapa,
                Duracion_Minima = source.Duracion_Minima,
                Duracion_Maxima = source.Duracion_Maxima
            };
        }
        ...
        private static ResponseOptModel GetBestResponse(IEnumerable<ResponseOptModel> results, int order, bool selectMax)
        {
            ...
            if (bestResponse == null)
                throw new Exception("No se encontró resultado optimizado");
        ...
        private TablaModel GetPeso_FinalEtapa9_10(RequestOptimizerModel objReqDataIni, TablaModel tModelAnt, ProductoModel prod, double edadTopeObjetivo)
        {
            ...
            if (prod.DuracionMin == 0 & prod.DuracionMax == 0)
            {
                ...
                tModel9 = GetDataRegistroCached(prod, objReqDataIni);
                return tModel9;
            }
        ...
        private VarMinMaxModel GetMinMaxValuesV2(RequestOptimizerModel objReqDataIni, VarMinMaxModel prodAnt, int identificador, Dictionary<int, double> remainingMinDurations, Dictionary<int, ProductoModel> productosByPosicion)
        {
            double pesoMin = 0;
            double pesoMax = 0;
            double peso2Min = 0;
            ...
            double edadTope = objReqDataIni.EdadVenta - remainingMinDurations[identificador];
            ProductoModel prod = productosByPosicion[identificador];
            double minEtapa = prod.DuracionMin;
            double maxEtapa = prod.DuracionMax;
            double maxEtapa2 = maxEtapa;
            bool isExactDuracion = false;
            ...
            for (double x = prodAnt.ValorMinimo; x <= 200; x++)
            {
                ...
                if (duracion >= minEtapa && duracion <= maxEtapa)
                {
                    ...
                    if (pesoMin == 0)
                    {
                        ...
                        if (isExactDuracion)
                        {
                            for (double z = x; z >= 0; z -= 0.01)
                            {
                                prod.PesoFinal = z;
                                tModel = GetDataRegistroCached(prod, objReqDataIni);
            ...
            {
                for (double x = prodAnt.ValorMaximo; x <= 200; x++)
                {
                    objReqDataIni.IsOptimizar = true;
        ...
        private ResponseOptimizerModel GetDataOptizerService(RequestOptimizerModel objReqData)
        {
